...
class DecayLearningRateApplication(SegmentationApplication):
    ...
    def set_iteration_update(self, iteration_message):
        ...
        if iteration_message.is_training:
            f = lambda x : self.max_lr*(np.cos(x/self.theta*np.pi)+1)/(x/self.beta+1)
            self.current_lr = f(current_iter)

            if current_iter % 10 == 0:
                loss = self.current_loss.eval()
                self.avg = (self.avg*self.cpt+loss)/(self.cpt+1)
                if loss < (1+self.tx)*self.avg and loss>(1-self.tx)*self.avg:
                    self.cpt +=1
                else:
                    self.cpt = 0
                self.prec_loss = loss
