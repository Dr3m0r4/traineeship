...
class DecayLearningRateApplication(SegmentationApplication):
    ...
    def __init__(self, net_param, action_param, is_training):
        ...
        self.tx = 0.2
        self.cpt = 0
        self.cycle = action_param.nb_cycle
        self.theta = float(action_param.max_iter/self.cycle)
    ...
    def set_iteration_update(self, iteration_message):
        ...
        if iteration_message.is_training:
            f = lambda x, y : self.init_lr*(np.cos(x/self.theta*np.pi+y*np.pi)+1)/(x/self.beta+1)
            if current_iter>0 and f(current_iter-1, self.count)==0:
                self.count+=1
